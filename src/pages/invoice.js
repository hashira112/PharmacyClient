...
export default function Invoice() {
    ...
    const salesAdd = e =>{
        ...
        if(data.name !== '' && list.total !== 0){ 
            console.log(data)
            axios.post('http://localhost:8080/api/sales',{"name":data.name,"amount":list.total,"date":data.date,"products":data.products})
    ...
    const [list, setList] = useState({data:[],quan:'',price:null,name:null,total:0 })
